...
public class NBTEditor {
    ...
    private static Map<Class<?>, Class<?>> NBTClasses;
    private static Map<Class<?>, Field> NBTTagFieldCache;
        ...
        for (int index = 0; index < keys.length; index++) {
            ...
            if (index + 1 == keys.length) {
                if (key == null) {
                    ...
                    if (notCompound == null) {
                        getMethod("listRemove").invoke(compound, key);
                    } else {
                        getMethod("setIndex").invoke(compound, key, notCompound);
                    }
                } else {
                    if (notCompound == null) {
                        getMethod("remove").invoke(compound, key);
                    } else {
            ...
            if (compound == null || key == null) {
                ...
                if (oldCompound.getClass().getSimpleName().equals("NBTTagList")) {
                    ...
                    if (notCompound == null) {
                        getMethod("remove").invoke(oldCompound, key);
                    } else {
